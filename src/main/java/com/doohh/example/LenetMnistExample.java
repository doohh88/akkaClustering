...
public class LenetMnistExample {
	...
	public static void main(String[] args) throws Exception {
		...
		MultiLayerConfiguration.Builder builder = new NeuralNetConfiguration.Builder().seed(seed).iterations(iterations)
				...
				.momentum(0.9).list(4)
				.layer(0,
						new ConvolutionLayer.Builder(5, 5).nIn(nChannels).stride(1, 1).nOut(20).dropOut(0.5)
								.activation("relu").build())
				.layer(1,
						new SubsamplingLayer.Builder(SubsamplingLayer.PoolingType.MAX).kernelSize(2, 2).stride(2, 2)
								.build())
				.layer(2, new DenseLayer.Builder().activation("relu").nOut(500).build())
				.layer(3, new OutputLayer.Builder(LossFunctions.LossFunction.NEGATIVELOGLIKELIHOOD).nOut(outputNum)
		 ...
		 * 
		 * 
		 * log.info("Train model...."); model.setListeners(new
		 * ScoreIterationListener(1)); for( int i=0; i<nEpochs; i++ ) {
		 * model.fit(mnistTrain); log.info("*** Completed epoch {} ***", i);
		 * 
		 ...
		 * model.output(ds.getFeatureMatrix()); eval.eval(ds.getLabels(),
		 * output); } log.info(eval.stats()); mnistTest.reset(); } log.info(
		 * "****************Example finished********************");
		 */
	}
}
